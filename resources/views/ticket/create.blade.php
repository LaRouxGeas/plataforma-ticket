...
@section('title', 'PTT - Ticket - Novo Ticket')

@section('content')
	<div id="idFundoJanela"></div>
	<h1 class="titulo-site">Novo ticket</h1>
	<form class="" method="POST" action="/pttgcc/ticket" enctype="multipart/form-data">
		{{ method_field('POST') }}
		{{ csrf_field() }}
		...
		<div class="campo-form campo-divisao">
			<div class="campo-divisao-75">
				...
				<input id="titulo" type="text" name="tic_titulo" class="input {{$errors->has('tic_titulo') ? 'erro-input' : ''}}" value="{{old('tic_titulo')}}">
			...
			<div class="campo-divisao-25 campo-data">
				...
				<input id="datapicker" type="text" name="tic_validade" readonly class="input {{$errors->has('tic_validade') ? 'erro-input' : ''}}" value="{{old('tic_validade')}}" >
		...
		<div class="campo-form">
			...
			<textarea id="descricaoTicket" name="tic_descricao" class="textarea {{$errors->has('tic_descricao') ? 'erro-input' : ''}}">{{old('tic_descricao')}}</textarea>
		...
		<div class="campo-form">
			...
			<div id="comunicacaoTicket" class="lista-itens-coluna {{$errors->has('comunicacoes') ? 'erro-div' : ''}}">
				...
				<div class="checkbox-opcao">
					<input id="checkCom{{$comunicacao->com_id}}" type="checkbox" name="comunicacoes[{{$comunicacao->com_id}}]" class="input-hidden" value="{{$comunicacao->com_id}}" {{old('comunicacoes.'.$comunicacao->com_id)?'checked':''}}>
			...
			@endif
			<div class="campo-form">
				<button id="outros" class="botao-enviar" type="button" data-nome="comunicacoes" data-rota="comunicacao" data-pre="com" data-nomecampo="Meio de comunicação">Outros</button>
			</div>
		</div>
		...
		<div class="campo-form">
			...
			<div id="empresaTicket" class="lista-itens-coluna {{$errors->has('empresas') ? 'erro-div' : ''}}">
				...
				<div class="checkbox-opcao">
					<input id="checkEmp{{$empresa->emp_id}}" type="checkbox" name="empresas[{{$empresa->emp_id}}]" class="input-hidden" value="{{$empresa->emp_id}}" {{old('empresas.'.$empresa->emp_id)?'checked':''}}>
		...
		<div class="campo-form">
			...
			<div id="regiaoTicket" class="lista-itens-coluna {{$errors->has('regioes') ? 'erro-div' : ''}}">
				...
				<div class="checkbox-opcao">
					<input id="checkReg{{$regiao->reg_id}}" type="checkbox" name="regioes[{{$regiao->reg_id}}]" class="input-hidden" value="{{$regiao->reg_id}}" {{old('regioes.'.$regiao->reg_id)?'checked':''}}>
		...
		<div class="campo-form">
			...
			<div id="produtoTicket" class="lista-itens-coluna {{$errors->has('produtos') ? 'erro-div' : ''}}">
				...
				<div class="checkbox-opcao">
					<input id="checkPro{{$produto->pro_id}}" type="checkbox" name="produtos[{{$produto->pro_id}}]" class="input-hidden" value="{{$produto->pro_id}}" {{old('produtos.'.$produto->pro_id)?'checked':''}}>
		...
		<div class="campo-form">
			...
			<div id="publico_alvoTicket" class="lista-itens-coluna {{$errors->has('publicosAlvos') ? 'erro-div' : ''}}">
				...
				<div class="checkbox-opcao">
					<input id="checkPua{{$publicoAlvo->pua_id}}" type="checkbox" name="publicosAlvos[{{$publicoAlvo->pua_id}}]" class="input-hidden" value="{{$publicoAlvo->pua_id}}" {{old('publicosAlvos.'.$publicoAlvo->pua_id)?'checked':''}}>
		...
		<div class="campo-form">
			...
			<div id="aprovadorTicket" class="lista-itens {{$errors->has('fk_aprovador') ? 'erro-div' : ''}}">
				...
				<div class="checkbox-opcao">
					<input id="radioApr{{$aprovador->id}}" type="radio" name="fk_aprovador" class="input-hidden" value="{{$aprovador->id}}" {{old('fk_aprovador')==$aprovador->id?'checked':''}}>
		...
		<div class="campo-form">
			...
			<textarea id="comentarioSolicitante" name="tic_obs_solicitante" class="textarea {{$errors->has('tic_obs_solicitante')?'erro-input':''}}">{{old('tic_obs_solicitante')}}</textarea>
		...
		<div class="campo-form">
			...
			<textarea name="tic_obs_aprovador" class="textarea" disabled></textarea>
		</div>
