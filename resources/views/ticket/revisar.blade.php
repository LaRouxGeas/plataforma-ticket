...
@section('title', 'PTT - Ticket '.$ticket->tic_id.' - Aprovar')

@section('content')
	<div id="idFundoJanela"></div>
	<h1 class="titulo-site">Ticket n. {{$ticket->tic_id}}</h1>
	<form class="" method="POST" action="/pttgcc/ticket/{{$ticket->tic_id}}/aprovar">
		{{ method_field('PATCH') }}
		{{ csrf_field() }}
		...
		<div class="campo-form">
			...
			@if ($ticket->tic_fk_estado == 4 || $ticket->tic_fk_estado == 5 || $ticket->tic_fk_estado == 6)
			<textarea name="tic_obs_aprovador" class="textarea {{$errors->has('tic_obs_aprovador') ? 'erro-input' : ''}}" disabled>{{$ticket->tic_observacao_aprovador}}</textarea>
			@else
			<textarea name="tic_obs_aprovador" class="textarea {{$errors->has('tic_obs_aprovador') ? 'erro-input' : ''}}">{{$ticket->tic_observacao_aprovador}}</textarea>
			@endif
		</div>
		...
		<div class="campo-form">
			<div class="lista-botoes">
				...
				@if ($ticket->tic_fk_estado != 4 && $ticket->tic_fk_estado != 5 && $ticket->tic_fk_estado != 6)
					<input type="submit" class="botao-enviar" name="enviar" value="Salvar">
					<input type="submit" class="botao-enviar" name="enviar" value="Não aprovado">
					<input type="submit" class="botao-enviar" name="enviar" value="Aprovado">
				@endif
			</div>
		</div>
	</form>
